...
if __name__ == '__main__':
  ...
  for line in tqdm(fread.readlines()):
    ...
    count_dict = collections.Counter(labels)
    # {0: count_for_0, 1: count_for_1, ...}
    num_frames = len(labels)  # Total number of frames
    # normalize the frequency by dividing with frame number
    list_freq = [count_dict.get(i, 0) / num_frames for i in range(args.cluster_num)]
